...
/*		vectorOfItems.push_back(items("rubik's cube", 9.99, 1, 2, 5));
		vectorOfItems.push_back(items("spalding basketball", 24.99, 2, 4, 1));
		vectorOfItems.push_back(items("coloring book", 4.99, 4, 1, 2));
		vectorOfItems.push_back(items("yamaha keyboard", 1299.99, 4, 1, 5)); 
		vectorOfItems.push_back(items("item 5", 19.99, 5, 4, 2)); 
		vectorOfItems.push_back(items("la fitness subscription", 29.99, 1, 4, 4)); 
		vectorOfItems.push_back(items("netflix subscription", 7.99, 1, 2, 1)); 
		vectorOfItems.push_back(items("dance lessons", 19.95, 1, 2, 1)); 
*/ 
		vectorOfItems.push_back(items("paintbrushes", 10, 5, 1, 2));
	   vectorOfItems.push_back(items("ball", 100, 1, 5, 0));
	   vectorOfItems.push_back(items("spaceship", 20, 2, 0, 5));
	   vectorOfItems.push_back(items("john cena", 100000000, 5, 5, 5));
	   vectorOfItems.push_back(items("john cena's twin", 100000000, 5, 5, 5));
	vector<employees> vectorOfEmployees; //creates the vector of employees
	vector<items> recommendedItems;
			...
			while (secondinput != 1 && secondinput != 2 && secondinput != 3 && keepgoing) {
				...
				if (secondinput == 1) {
					...
					if (!isInStock) {
						...
						cout << input << "'s are " << vectorOfItems[index].getItemPrice() << " each. Purchase? (y/n)" << endl;
