...
void recGift(vector<items> & vectorOfItems, vector<items> & recommendedItems, Customer & user) {
	...
	cout << endl << "Would you rather 1. visit the baseball Hall of Fame or 2. attend the World Cup?" << endl;
	userpref = 0;
	getline(cin, userchoice);
	userpref = atoi(userchoice.c_str());
	while (!isNumber(userchoice) || (userpref != 1 && userpref != 2))
	...
	while (firstInput != 1 && firstInput != 2 && firstInput != 3) { //input validation for the first menu choice
		...
		if (firstInput == 1) {
			...
			while (secondInput != 1 && secondInput != 2 && secondInput != 3)
			{
				...
				if (secondInput == 1)
				{
					...
					if (isAuthenticated) {
						...
						while (loggedInInput != 1 && loggedInInput != 2 && loggedInInput != 3 && loggedInInput != 4 && loggedInInput != 5) {
							isNum3 = true;
							//the following if statement only prints "Login successful." upon the first time login
							...
							{ //RESTOCK METHOD
								...
								{
									c = n[counter];
									if (isupper(c)) //if the character is uppercase, covert it to lowercase
									{
										n[counter] = tolower(c);
									}
									counter++;
								...
								if (!usingprevious)
								{
									...
									cout << "What is the cost of making this item?" << endl; // prompts user for cost
									cout << "$";
									getline(cin, costStr);
							...
							else if (loggedInInput == 4) //prints the total sales of the employee
							{
								cout << "Employee " << vectorOfEmployees[currentEmployee].getEmpName() << " has sold items with profits totaling $" << fixed << setprecision(2) << vectorOfEmployees[currentEmployee].getEmpTotalSales() << endl;
								loggedInInput = 0;
							}
				...
				{ //entry point for new account registration
					string name = "";
					string username = "";
					string password = "";
		...
		if (firstInput == 2) { //entry point for customer
			...
			while (secondinput != 1 && secondinput != 2 && secondinput != 3 && secondinput != 4 && secondinput != 5 && secondinput != 6 && keepgoing) {
				...
				if (secondinput == 1)
				{ //entry point for item purchase within customer class
					purchaseItem(vectorOfItems, *user);
					secondinput = 0;
				}
				else if (secondinput == 2)
				{ //entry point for the questionnaire
				...
				else if (secondinput == 4) //displays current budget
				{
					cout << "You're current reminaing budget is $" << fixed << setprecision(2) << user->getCustomerBudget() << endl;
					cout << endl;
					secondinput = 0;
				}
				else if (secondinput == 5) //updates current budget
				{
				...
				{ //exit function to return to employee/customer select screen
					firstInput = -1;
					cout << endl;
					cout << "Welcome to the shop! Are you an employee or customer?" << endl;
					cout << "1. Employee" << endl;
				...
				{
					cerr << endl << "Error, not a valid choice. " << endl; //error message for user inputting an invalid choice
				}
		...
		if (firstInput == 3)
		{ //this is how to the user exists the shop
			...
			{ //input validation for the exitOption variable
		...
		{
			//if the input from the check fails, simply do nothing
		}
		else if (!(firstInput == 1 || firstInput == 2 || firstInput == 3)) //if the input is not one of the desired from the menu, print out an error.
		{
